<#
.SYNOPSIS
    Change users password, remove the PasswordNeverExpires flag, and set ChangePasswordAtLogon to force a password change for all Admin accounts that haven't logged in for 90 days or more
.DESCRIPTION
    Change users password, remove the PasswordNeverExpires flag, and set ChangePasswordAtLogon to force a password change for all Admin accounts that haven't logged in for 90 days or more
.INPUTS
...
$admins = Get-ADUser -Filter {(adminCount -like "1") -and (enabled -eq $true)} | Where-Object {$_.lastlogondate -le $date}
...
Function Cleanup {
    foreach ($uName in $admins) {
        Set-ADAccountPassword -Identity $uName.samaccountname -NewPassword (Pass)
		...
		foreach ($grpMembership in $grpMemberships.name) {
			...
			"$($uName.name) is a member of: $grpMembership" | out-file -filepath Disabled-Admin_Accounts-$(get-date -f yyy-MM-dd).txt -Append
			Write-Output ""
			}
